...
The run path uses AsyncFPLClient. This client wraps the same
sync HTTP layer (requests session, retry logic, pacing gate) and
exposes the same FPL endpoint methods.

This module is HTTP-only — it has no knowledge of transform models
or orchestration policy.

Usage:
    from fpl_ingest.extract.http import FPLClient
