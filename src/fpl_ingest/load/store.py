"""SQLite persistence layer for the compiled public contract tables.

``SQLiteStore`` manages a single shared WAL-mode connection within transaction
blocks, performs idempotent upserts using compiled contract metadata (primary
keys, unique constraints), migrates new nullable columns without data loss, and
maintains the ``_runs`` audit table and ``_metadata`` freshness store. It does
not contain schema definition logic — that lives in the schema sub-package.
"""
...
class SQLiteStore:
    ...
    def bulk_upsert(
        ...
        *,
        ...
        """Upsert rows in bulk. Uses ON CONFLICT DO UPDATE when conflict_target is provided."""
        if not rows:
            return 0
    ...
    def upsert_models(
        ...
        *,
        ...
        """Validate raw dicts against schema and upsert into SQLite. Returns (written, skipped)."""
        rows: List[tuple] = []
    ...
    _STAGE_LINEAGE_DDL = (
        "CREATE TABLE IF NOT EXISTS _stage_lineage ("
        "  id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "  started_at TEXT NOT NULL,"
        "  stage TEXT NOT NULL,"
        "  artifact_path TEXT,"
    ...
    def setup_runs_table(self) -> None:
        ...
        self._exec(self._STAGE_LINEAGE_DDL)
    ...
    def record_stage_lineage(
        ...
        *,
        ...
        """Persist raw-artifact to output-table lineage for a completed stage."""
        pairs = [
            (artifact_path, output_table)
            for artifact_path in (artifact_paths or (None,))
            for output_table in (output_tables or (None,))
        ]
        if not pairs:
            return

        sql = (
            "INSERT INTO _stage_lineage (started_at, stage, artifact_path, output_table) "
            "VALUES (?, ?, ?, ?)"
        )
        own_conn = self._active_conn is None
        effective = self._active_conn or self._get_connection()
        try:
            effective.executemany(
                sql,
                [(started_at, stage, artifact_path, output_table) for artifact_path, output_table in pairs],
            )
            if own_conn:
                effective.commit()
        finally:
    ...
    def run_integrity_checks(self) -> None:
        """Run post-run cross-table integrity assertions.

        Delegates to the named check functions in load.integrity so each
        check is independently testable.  Hard violations raise
        IntegrityViolation; the player-count mismatch is a warning only.

        Should only be called after a fully successful run — the runner
        enforces this.
        """
        from fpl_ingest.load.integrity import run_integrity_checks as _run_checks
