"""Core schema compiler that derives all enforcement artifacts from the contract.

Walks each ``TableContract`` and its Pydantic model to produce ``CompiledTable``
objects containing CREATE TABLE SQL, index SQL, and column metadata. The
top-level ``compile_contract()`` function is LRU-cached so the compilation
happens once per process. Downstream callers (store, DDL generator, validation
rules, test fixtures) all consume ``CompiledContract`` rather than the raw
``TableContract`` declarations.
"""
...
class CompiledColumn:
    ...
    def alter_sql(self) -> str:
        """Return the ALTER TABLE ADD fragment, or raise if the column requires a manual migration."""
        if self.primary_key:
...
def compile_contract() -> CompiledContract:
    ...
    from fpl_ingest.schema.validation import generate_validation_rules
    from fpl_ingest.schema.definition import PUBLIC_TABLES, SCHEMA_VERSION
