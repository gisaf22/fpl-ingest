"""Public schema contract — the single source of truth for the SQLite schema.

Declares all eight public tables as ``TableContract`` instances and compiles
them into DDL, validation rules, and a versioned JSON artifact via the
``schema`` sub-package. Consumers compare their live database against the
artifact to detect missing tables, type mismatches, and constraint drift.
This module does not perform I/O directly; write/validate operations delegate
...
CONTRACT_ARTIFACT_PATH = Path(__file__).resolve().parents[3] / "artifacts" / "contract" / "schema_contract.json"
...
PUBLIC_TABLES: tuple[TableContract, ...] = (
    TableContract(
        name="players",
        model=PlayerModel,
        description="Current-season player latest-state dataset; one row per player.",
        grain="one row per player",
        field_notes={
            "element_type": "1=GK, 2=DEF, 3=MID, 4=FWD.",
            "selected_by_percent": "Latest-state ownership percentage, not historical ownership.",
...
def write_contract_artifact(out_path: Path | None = None) -> Path:
    ...
    from fpl_ingest.schema.compiler import write_contract_artifact as _write_contract_artifact
...
def validate_contract(db_path: str | Path | None = None) -> ValidationResult:
    ...
    Also warns if the compiled artifact at CONTRACT_ARTIFACT_PATH has a
    different schema_version than SCHEMA_VERSION declared in this module.
    A mismatch means ``fpl-ingest schema export`` has not been run since
    the last code change — the artifact is stale.
    """
    import json as _json

    from fpl_ingest.schema import compile_contract
    from fpl_ingest.schema.validation import validate_contract_db

    result = validate_contract_db(compile_contract(), db_path)

    if CONTRACT_ARTIFACT_PATH.exists():
        try:
            artifact = _json.loads(CONTRACT_ARTIFACT_PATH.read_text(encoding="utf-8"))
            artifact_version = artifact.get("schema_version")
            if artifact_version and artifact_version != SCHEMA_VERSION:
