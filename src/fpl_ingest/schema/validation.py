...
Fetches bootstrap-static, fixtures, and a sample of player histories, then
asserts the presence of required top-level keys and per-record fields.
Raises ``SmokeTestFailure`` on any structural mismatch so CI and operators
are alerted before a full ingestion run captures a payload the rest of the
pipeline no longer recognises.

This is the source-shape validation for the raw-capture boundary (strategy
doc B.1/B.2) and is the only half of the old ``schema.validation`` module
that survives the SQLite removal — the contract-vs-live-database validation
that used to share this file validated a database that no longer exists.
"""
