...
from fpl_ingest.extract.http.rate_config import MAX_RATE
from fpl_ingest.extract.http.rate_limiter import NoopRateLimiter, TokenBucketLimiter
...
def test_context_manager_opens_and_closes_session():
    """Session must be usable inside the context manager and closed on exit."""
    close_called = False
    ...
    async def _run() -> None:
        ...
        async def _spy_close(self_: object) -> None:
            nonlocal close_called
            close_called = True

        resp = _mock_response(200, json_data=BOOTSTRAP_DATA)
        with (
            patch("aiohttp.ClientSession.get", return_value=resp),
            patch.object(_aiohttp.ClientSession, "close", _spy_close),
        ):
            async with _make_client() as client:
                result = await client.get_bootstrap()
            assert result is not None  # request succeeds inside the context manager

    asyncio.run(_run())
    assert close_called, "ClientSession.close must be called on __aexit__"


def test_token_bucket_rate_is_clamped_inside_async_client():
    # _rate_limiter is private but is the only way to observe the enforced ceiling;
    # AsyncFPLClient exposes no public rate accessor.
    client = AsyncFPLClient()
    assert isinstance(client._rate_limiter, TokenBucketLimiter)
    assert client._rate_limiter.rate <= MAX_RATE


def test_logs_when_provided_token_bucket_exceeds_max_rate():
    with patch("fpl_ingest.extract.http.client.logger") as logger:
        client = AsyncFPLClient(rate_limiter=TokenBucketLimiter(rate=MAX_RATE + 5))

    # _rate_limiter is private but is the only way to observe the enforced ceiling;
    # AsyncFPLClient exposes no public rate accessor.
    assert client._rate_limiter.rate == MAX_RATE
    logger.warning.assert_called_once()  # call count justified: exactly one clamping warning expected
...
def test_concurrency_slot_released_before_retry_sleep():
    ...
    A spy wraps asyncio.sleep (as used in client) and records whether
    ...
    async def _run():
        ...
        with (
            ...
            patch("fpl_ingest.extract.http.client.asyncio.sleep", side_effect=_tracking_sleep),
        ):
            async with client:
                result = await client._fetch_with_retries("https://example.com/")
