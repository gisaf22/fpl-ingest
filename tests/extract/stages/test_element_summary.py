...
@pytest.mark.asyncio
async def test_ingest_player_histories_writes_a_raw_object_per_player(tmp_path):
    writer = _writer(tmp_path)
    responses = {1: _raw(_payload(1), player_id=1), 2: _raw(_payload(2), player_id=2)}

    outcome = await ingest_player_histories(_client(responses), writer, [1, 2])
...
def test_raw_endpoint_puts_player_id_before_extraction_date():
    """strategy doc A.3: {player_id} is part of the endpoint segment."""
    assert raw_endpoint(115) == "element-summary/115"


@pytest.mark.asyncio
async def test_ingest_player_histories_sidecar_carries_capture_metadata(tmp_path):
    writer = _writer(tmp_path)
    raw = _raw(_payload(7), player_id=7, attempt_count=3)

    await ingest_player_histories(_client({7: raw}), writer, [7])
...
@pytest.mark.asyncio
async def test_all_players_share_one_run_manifest(tmp_path):
    writer = _writer(tmp_path)
    responses = {pid: _raw(_payload(pid), player_id=pid) for pid in (1, 2, 3)}

    outcome = await ingest_player_histories(_client(responses), writer, [1, 2, 3])
...
def test_validate_element_summary_shape_flags_each_structural_failure(raw_kwargs, expected_failure):
...
def test_validate_element_summary_shape_passes_a_well_formed_payload():
...
def test_validate_element_summary_shape_accepts_an_empty_history_list():
...
def test_validate_element_summary_shape_samples_rather_than_scans():
...
async def test_shape_failure_still_writes_the_payload_and_flags_the_manifest(tmp_path):
    ...
    outcome = await ingest_player_histories(_client({1: raw}), writer, [1])
...
@pytest.mark.asyncio
async def test_one_bad_player_does_not_discount_the_good_ones(tmp_path):
    writer = _writer(tmp_path)
    responses = {
...
@pytest.mark.asyncio
async def test_non_strict_failure_captures_the_other_players(tmp_path):
    writer = _writer(tmp_path)
    responses: dict = {1: FPLClientError("network down"), 2: _raw(_payload(2), player_id=2)}

    outcome = await ingest_player_histories(_client(responses), writer, [1, 2], strict=False)
...
async def test_strict_failure_writes_nothing_and_trips_the_fail_fast_sentinel(tmp_path):
    ...
    outcome = await ingest_player_histories(
        _client(responses), writer, [1, 2], strict=True, execution_state=state
    )
...
async def test_fail_fast_skips_the_capture_entirely(tmp_path):
    ...
    outcome = await ingest_player_histories(client, writer, [1], execution_state=state)
...
@pytest.mark.asyncio
async def test_no_player_ids_captures_nothing(tmp_path):
    client = _client({})

    outcome = await ingest_player_histories(client, _writer(tmp_path), [])
...
@pytest.mark.asyncio
async def test_ingest_player_histories_has_no_force_parameter(tmp_path):
