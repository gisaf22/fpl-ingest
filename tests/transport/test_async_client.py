...
from fpl_ingest.transport.rate_limiter import NoopRateLimiter, TokenBucketLimiter
...
def test_logs_when_provided_token_bucket_exceeds_max_rate():
    with patch("fpl_ingest.transport.async_client.logger") as logger:
...
def test_concurrency_slot_released_before_retry_sleep():
    ...
    async def _run():
        ...
        with (
            ...
            patch("fpl_ingest.transport.async_client.asyncio.sleep", side_effect=_tracking_sleep),
        ):
            async with client:
                result = await client._fetch_with_retries("https://example.com/")
