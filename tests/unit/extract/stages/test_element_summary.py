...
class TestSkipFlag:
    """``--skip-player-histories``: the stage does nothing at all.

    The flag exists so a schedule that fires often enough for re-capturing
    ~650 already-settled players to be waste can run every other stage
    without this one. What it must never do is let such a run consume the
    settlement transition — see the guard at the top of
    ``ingest_player_histories`` for why its return sits above
    ``_settlement_refetch_event`` rather than anywhere below it.
    """

    @pytest.mark.asyncio
    async def test_skip_returns_a_clean_zeroed_result_without_fetching(self, tmp_path):
        """All zeros, not `skipped=len(player_ids)`: `classify_run` reads any
        skipped > 0 as FAILED_PARTIAL, which exits 1 and fails the schedule."""
        client = _client({})

        outcome = await ingest_player_histories(
            client, _writer(tmp_path), [1, 2], [_event(1, finished=False)],
            event_finality=_provisional(1),
            skip_player_histories=True,
        )

        client.get_element_summary_raw.assert_not_called()
        result = outcome.result
        assert (
            result.fetched, result.validated, result.written, result.skipped, result.errors
        ) == (0, 0, 0, 0, 0)
        assert result.is_clean
        assert classify_run_from_results([result], strict_mode=False) == RUN_STATUS_SUCCESS

    @pytest.mark.asyncio
    async def test_skip_never_reaches_the_settlement_decision(self, tmp_path, monkeypatch):
        """Pins the ordering itself, not just its consequence: the guard returns
        before the settlement machinery is reached at all."""
        def _unreachable(*args, **kwargs):
            raise AssertionError(
                "_settlement_refetch_event was reached under skip_player_histories"
            )

        monkeypatch.setattr(
            element_summary_stage, "_settlement_refetch_event", _unreachable
        )

        outcome = await ingest_player_histories(
            _client({}), _writer(tmp_path), [1], [_event(1, finished=True)],
            event_finality=_settled(1),
            skip_player_histories=True,
        )

        assert outcome.result.is_clean

    @pytest.mark.asyncio
    async def test_skip_leaves_the_settlement_transition_still_owed(self, tmp_path):
        """The consequence that matters. A skipped run writes no marker, so the
        next run that does capture still performs the forced re-fetch — the
        ratification-only fields are not frozen at their provisional zeroes."""
        player_id = 1
        events = [_event(1, finished=True)]
        payload = payload_bytes("element_summary_settled_capture")
        marker = tmp_path / "raw" / "fpl" / "_settlement" / "element-summary" / "1"

        # A provisional run first, so a capture exists and existence-based
        # skipping would apply from here on once the gameweek reads settled.
        await ingest_player_histories(
            _client({player_id: _raw(body=payload, player_id=player_id)}),
            _writer(tmp_path), [player_id], events,
            event_finality=_provisional(1),
        )
        ...
        skipped_client.get_element_summary_raw.assert_not_called()
        assert not marker.exists(), "a skipped run must not claim the settlement"

        settlement_client = _client({player_id: _raw(body=payload, player_id=player_id)})
        settlement_outcome = await ingest_player_histories(
            settlement_client,
            _writer_with_run_id(tmp_path, "20260824T100000Z-abc125"),
            [player_id], events,
        ...
        settlement_client.get_element_summary_raw.assert_called_once()
        assert settlement_outcome.result.fetched == 1
        assert marker.exists(), "the deferred transition must still be recorded"
